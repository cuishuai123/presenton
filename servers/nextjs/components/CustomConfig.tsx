    ...
    <div className="space-y-6">
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-2">
          {t('settings.openaiCompatibleUrl')}
        </label>
        <div className="relative">
          <input
            type="text"
            required
            placeholder={t('settings.enterYourUrl')}
      ...
      <div className="mb-4">
        ...
        <div className="relative">
          <input
            type="text"
            required
            placeholder={t('settings.enterYourApiKey')}
            className="w-full px-4 py-2.5 outline-none border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
          ...
          >
            {customModelsLoading ? (
              <div className="flex items-center justify-center gap-2">
                ...
                {t('settings.checkingForModels')}
              </div>
            ) : (
              t('settings.checkForAvailableModels')
            )}
          </button>
        </div>
      )}
      ...
      {customModelsChecked && customModels.length === 0 && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800">
            {t('settings.noModelsFound')} {t('settings.noModelsFoundDesc')}
          </p>
        </div>
      )}
        ...
        <div className="mb-4">
          <div className="mb-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-sm text-amber-800">
              <strong>{t('settings.importantToolCalls').split(':')[0]}:</strong> {t('settings.importantToolCalls').split(':')[1]}
            </p>
          </div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
          ...
          <div className="w-full">
            ...
            >
              <PopoverTrigger asChild>
                ...
                >
                  <span className="text-sm font-medium text-gray-900">
                    {customModel || t('settings.selectAModel')}
                  </span>
              ...
              >
                <Command>
                  <CommandInput placeholder={t('settings.searchModel')} />
                  <CommandList>
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-gray-500 flex items-center gap-2">
          <span className="block w-1 h-1 rounded-full bg-gray-400"></span>
          {t('settings.disableThinkingDesc')}
        </p>
      </div>
    </div >
  );
