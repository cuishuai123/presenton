...
const SupportingDoc = ({ files, onFilesChange }: SupportingDocProps) => {
    ...
    const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (hasPdf && droppedFiles.some(file => file.type === 'application/pdf')) {
            toast.error(t('upload.multiplePdfNotAllowed'), {
                description: t('upload.multiplePdfNotAllowedDesc'),
            });
            return;
        }
        ...
        if (validFiles.length > 0) {
            ...
            toast.success(t('upload.filesSelected'), {
                description: `${validFiles.length} ${t('upload.filesAdded')}`,
            })
        }
    }
    ...
    return (
        <div className="w-full">
            <h2 className="text-[#444] font-instrument_sans pt-4 text-lg mb-4">{t('upload.supportingDocuments')}</h2>
            <div
