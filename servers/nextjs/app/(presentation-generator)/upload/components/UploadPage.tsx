...
const UploadPage = () => {
  ...
  const [config, setConfig] = useState<PresentationConfig>({
    slides: "8",
    language: LanguageType.ChineseSimplified, // 默认使用简体中文
    prompt: "",
  ...
  const validateConfiguration = (): boolean => {
    if (!config.language || !config.slides) {
      toast.error(t('upload.selectSlidesAndLanguage'));
      return false;
    }

    if (!config.prompt.trim() && files.length === 0) {
      toast.error(t('upload.noPromptOrDocument'));
      return false;
    }
    return true;
  };
