...
const SettingsPage = () => {
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      toast.info(t('settings.configurationSaved'));
      setButtonState(prev => ({
        ...prev,
        isLoading: false,
        isDisabled: false,
              ...
              {downloadProgress > 0 && (
                <div className="mb-4">
                  ...
                  <p className="text-sm text-gray-600 mt-2">
                    {downloadProgress}% {t('settings.complete')}
                  </p>
                </div>
              )}
                ...
                downloadingModel.status !== "pulled" && (
                  <div className="text-xs text-gray-500">
                    {downloadingModel.status === "downloading" &&
                      t('settings.downloadingModelFiles')}
                    {downloadingModel.status === "verifying" &&
                      t('settings.verifyingModelIntegrity')}
                    {downloadingModel.status === "pulling" &&
                      t('settings.pullingModelFromRegistry')}
                  </div>
                )}
              ...
              {downloadingModel.downloaded && downloadingModel.size && (
                <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                  <div className="flex justify-between text-xs text-gray-600">
                    ...
                    <span>
                      {t('settings.total')}: {(downloadingModel.size / 1024 / 1024).toFixed(1)}{" "}
                      MB
                    </span>
                  </div>
                </div>
