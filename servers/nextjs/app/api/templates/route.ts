    ...
    try {
        ...
        for (const templateName of templateDirectories) {
            try {
                ...
                let layoutFiles = templateFiles.filter(file => 
                    file.endsWith('.tsx') && 
                    !file.startsWith('.') && 
                    !file.includes('.test.') &&
                    !file.includes('.spec.') &&
                ...
                // Sort files numerically if ordered is true
                if (settings?.ordered === true) {
                    layoutFiles = layoutFiles.sort((a, b) => {
                        // Extract numbers from filenames (e.g., "Slide1.tsx" -> 1, "Slide10.tsx" -> 10)
                        ...
                        // If both have numbers, sort by number; otherwise sort alphabetically
                        if (numA !== 0 && numB !== 0) {
                            return numA - numB
                        }
