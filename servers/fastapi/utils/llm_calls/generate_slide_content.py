...
async def get_slide_content_from_type_and_outline(
    ...
    print(f"INFO: Starting LLM call for slide layout '{slide_layout.id}', outline: {outline.content[:100]}...")
    ...
    try:
        ...
        print(f"INFO: LLM call completed for layout '{slide_layout.id}' in {elapsed:.2f}s")
        return response

    except asyncio.TimeoutError:
        elapsed = (datetime.now() - start_time).total_seconds()
        print(f"ERROR: LLM call timeout after {elapsed:.2f}s for layout '{slide_layout.id}'")
        from fastapi import HTTPException
        raise HTTPException(
            status_code=504,
            detail=f"Slide generation timed out after {elapsed:.0f} seconds. Please try again with a simpler outline or different model."
        )
    except Exception as e:
        elapsed = (datetime.now() - start_time).total_seconds()
        print(f"ERROR: LLM call failed after {elapsed:.2f}s for layout '{slide_layout.id}': {str(e)}")
