...
async def prepare_presentation(
    ...
    print(f"INFO: [PREPARE] Request received for presentation {presentation_id}")
    print(f"INFO: [PREPARE] Total outlines: {len(outlines)}, Layout: {layout.name}")
    ...
    print(f"INFO: [PREPARE] Layout ordered: {layout.ordered}, total layouts: {total_slide_layouts}")

    if layout.ordered:
        print(f"INFO: [PREPARE] Using ordered layout structure")
        presentation_structure = layout.to_presentation_structure()
    else:
        print(f"INFO: [PREPARE] Calling LLM to generate structure for {total_outlines} slides...")
        start_time = datetime.now()
        try:
            # 添加180秒超时保护（8张slides可能需要较长时间）
            presentation_structure: PresentationStructureModel = await asyncio.wait_for(
                generate_presentation_structure(
            ...
            print(f"INFO: [PREPARE] Structure generation completed in {elapsed:.2f}s")
        except asyncio.TimeoutError:
            elapsed = (datetime.now() - start_time).total_seconds()
            print(f"ERROR: [PREPARE] Structure generation timeout after {elapsed:.2f}s")
            raise HTTPException(
                status_code=504,
                detail=f"Layout selection timed out after {elapsed:.0f} seconds. Please try with fewer slides or use an ordered template."
            )
        except Exception as e:
            elapsed = (datetime.now() - start_time).total_seconds()
            print(f"ERROR: [PREPARE] Structure generation failed after {elapsed:.2f}s: {str(e)}")
            traceback.print_exc()
            raise HTTPException(
                status_code=500,
    ...
    print(f"INFO: [PREPARE] Presentation prepared successfully with {len(presentation_structure.slides)} slides")
    print(f"INFO: [PREPARE] Presentation ready for streaming")
...
async def stream_presentation(
    ...
    print(f"INFO: [STREAM] Request received for presentation {id}")
    
    presentation = await sql_session.get(PresentationModel, id)
    print(f"INFO: [STREAM] Database query completed, presentation found: {presentation is not None}")
    
    if not presentation:
        raise HTTPException(status_code=404, detail="Presentation not found")
    if not presentation.structure:
        print(f"ERROR: [STREAM] Presentation {id} has no structure")
        raise HTTPException(
            status_code=400,
            detail="Presentation not prepared for stream",
        )
    if not presentation.outlines:
        print(f"ERROR: [STREAM] Presentation {id} has no outlines")
        raise HTTPException(
            status_code=400,
            detail="Outlines can not be empty",
        )

    print(f"INFO: [STREAM] Validation passed, initializing image generation service")
    image_generation_service = ImageGenerationService(get_images_directory())
    print(f"INFO: [STREAM] Image service initialized, entering inner generator")

    async def inner():
        print(f"INFO: [STREAM] Inner function started")
        
        print(f"INFO: [STREAM] Getting structure...")
        structure = presentation.get_structure()
        print(f"INFO: [STREAM] Structure obtained: {len(structure.slides) if structure else 0} slides")
        
        print(f"INFO: [STREAM] Getting layout...")
        layout = presentation.get_layout()
        print(f"INFO: [STREAM] Layout obtained: {layout.name if layout else 'None'}")
        
        print(f"INFO: [STREAM] Getting outline...")
        outline = presentation.get_presentation_outline()
        print(f"INFO: [STREAM] Outline obtained: {len(outline.slides) if outline else 0} slides")
        ...
        print(f"INFO: Starting slide generation for {total_slides} slides")
        
        for i, slide_layout_index in enumerate(structure.slides):
            slide_layout = layout.slides[slide_layout_index]
            print(f"INFO: Generating slide {i+1}/{total_slides}, layout: {slide_layout.id}")
            ...
            try:
                ...
                print(f"INFO: Slide {i+1}/{total_slides} generated successfully")
            except HTTPException as e:
                print(f"ERROR: Slide {i+1}/{total_slides} generation failed: {e.detail}")
                yield SSEErrorResponse(detail=e.detail).to_string()
                return
            except Exception as e:
                print(f"ERROR: Slide {i+1}/{total_slides} unexpected error: {str(e)}")
                import traceback
                traceback.print_exc()
                yield SSEErrorResponse(detail=f"Failed to generate slide {i+1}: {str(e)}").to_string()
                return
