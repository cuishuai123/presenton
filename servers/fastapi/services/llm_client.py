...
class LLMClient:
    def __init__(self):
        ...
        self._markdown_start_pattern = re.compile(r'^\s*```(?:json)?\s*\n?')
        self._markdown_end_pattern = re.compile(r'\n?\s*```\s*$')

    def _clean_llm_response(self, content: str) -> str:
        """
        统一清理LLM响应内容，移除思考标签、markdown标记等
        优化版本：减少字符串操作次数，提高解析效率
        """
        ...
        # 1. 移除 <think>...</think> 标签（qwq等模型会返回思维链）
        if '<think>' in content:
            # 使用预编译的正则，一次性移除所有think标签（支持嵌套）
            while '<think>' in content:
                content = self._think_pattern.sub('', content)
            content = content.strip()
            print(f"INFO: Removed <think> tags, size reduced: {original_length} -> {len(content)} bytes")
        
        # 2. 移除 markdown 代码块标记（```json 或 ```）
        if '```' in content:
            content = self._markdown_start_pattern.sub('', content)
            content = self._markdown_end_pattern.sub('', content)
            content = content.strip()
        
        # 3. 提取 tool call 格式中的 arguments（如果存在）
        if '"name"' in content and '"arguments"' in content:
            try:
                temp_parse = json.loads(content)
                if 'arguments' in temp_parse:
                    content = json.dumps(temp_parse['arguments'])
            except:
                pass
    ...
    async def _generate_openai_structured(
        ...
        # 打印响应摘要（避免打印超长内容影响性能）
        if content:
            content_preview = repr(content[:200]) + '...' if len(str(content)) > 200 else repr(content)
            print(f"INFO: OpenAI response at depth {depth}: {len(response.choices)} choices, "
                  f"content length: {len(str(content)) if content else 0} chars")
        ...
        if content:
            # 统一清理LLM响应内容（移除think标签、markdown标记等）
            if isinstance(content, str):
                original_len = len(content)
                content = self._clean_llm_response(content)
                # 只在内容被清理时打印简要信息
                if len(content) < original_len:
                    print(f"INFO: Cleaned LLM response at depth {depth}: {original_len} -> {len(content)} bytes")
